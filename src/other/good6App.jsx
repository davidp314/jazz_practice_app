...
const JazzGuitarTracker = () => {
  ...
  useEffect(() => {
    if (isTimerRunning && activeTask) {
      timerRef.current = setInterval(() => {
        ...
        if (sessionStartTime) {
          setSessionElapsedTime(Math.floor((new Date() - new Date(sessionStartTime)) / 1000));
        }
      ...
      // Even when timer is paused, update session elapsed time if session is active
      if (sessionStartTime) {
  ...
  }, [isTimerRunning, activeTask, sessionStartTime]);
  ...
  const createSession = (tasks) => {
    ...
    setTaskTimeSpent({});
    setSessionStartTime(null); // Will be set when first task starts
          ...
          {activeTask ? (
            <>
              ...
              <div className={`text-lg mb-1 transition-colors duration-300 ${
                ...
                Session Time: <span className="text-2xl font-bold">{formatTime(getSessionElapsedTime())}</span>
              </div>
              <div className={`text-sm mb-1 transition-colors duration-300 ${
                isDarkMode ? 'text-blue-400' : 'text-gray-600'
              }`}>
                Task Time: {formatTime(getTaskElapsedTime(activeTask.id))} / {formatTime(activeTask.timeAllocated * 60)}
                {getTaskElapsedTime(activeTask.id) > activeTask.timeAllocated * 60 && (
                  <span className="ml-2 text-orange-500 font-medium">
                    (+{formatTime(getTaskElapsedTime(activeTask.id) - activeTask.timeAllocated * 60)} over)
                  </span>
                )}
              </div>
              <div className={`text-lg mb-4 transition-colors duration-300 ${
                ...
                {timeRemaining > 0 ? `${formatTime(timeRemaining)} remaining` : 'Time completed - continue or switch tasks'}
              </div>
