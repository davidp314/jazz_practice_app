...
const JazzGuitarTracker = () => {
      ...
      timerRef.current = setInterval(() => {
        setTimeRemaining(prev => {
          if (prev <= 1) {
            ...
            const currentIndex = currentSession.tasks.findIndex(t => t.id === activeTask.id);
            if (currentIndex < currentSession.tasks.length - 1) {
              const nextTask = currentSession.tasks[currentIndex + 1];
              setActiveTask(nextTask);
              setTimeRemaining(nextTask.timeAllocated * 60);
            } else {
              setActiveTask(null);
              endSession();
            }
  ...
  }, [isTimerRunning, timeRemaining, currentSession, activeTask]);
  ...
  const startTimer = (task) => {
    setActiveTask(task);
    setTimeRemaining(task.timeAllocated * 60);
...
const SessionSetup = ({ standards, otherWork, onCreateSession, onCancel, getRepertoireRotation, isDarkMode, toggleDarkMode }) => {
  ...
  const addRepertoireTask = () => {
    if (nextRepertoire) {
      ...
      const task = {
        id: Date.now().toString(),
        name: `${nextRepertoire.name} (Repertoire)`,
        type: TASK_TYPES.STANDARD,
        timeAllocated: time,
        standardId: nextRepertoire.id
      };
      setSessionTasks([...sessionTasks, task]);
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className={`p-4 rounded-lg transition-colors duration-300 ${
            ...
            <p className={`text-sm mb-2 transition-colors duration-300 ${
              ...
              Next: {nextRepertoire ? nextRepertoire.name : 'None active'}
            </p>
      ...
      <div className={`rounded-lg shadow-lg p-6 transition-colors duration-300 ${
          ...
          <div className={`p-6 rounded-lg mb-6 text-center transition-colors duration-300 ${
            ...
            <div className="flex justify-center gap-4">
              {!isTimerRunning ? (
                ...
                >
                  <Play size={20} />
                  {timeRemaining === activeTask.timeAllocated * 60 ? 'Start' : 'Resume'}
                </button>
              ) : (
                <button
        ...
        <div className="mb-6">
          ...
          <div className="space-y-3">
            {session.tasks.map((task, index) => (
              <div 
                key={task.id} 
                className={`flex items-center gap-4 p-3 rounded-lg border transition-colors duration-300 ${
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className={`text-sm capitalize transition-colors duration-300 ${
                    ...
                    {task.type.replace('_', ' ')} • {task.timeAllocated} minutes
                  </div>
                </div>
                {!activeTask && (
                  <button
        ...
        <div className={`p-4 rounded-lg transition-colors duration-300 ${
          ...
          <div className={`flex justify-between text-sm mb-2 transition-colors duration-300 ${
            ...
            <span>{currentTaskIndex + 1} of {session.tasks.length} tasks</span>
          </div>
          <div className={`w-full rounded-full h-2 transition-colors duration-300 ${
            ...
            <div 
              className="bg-blue-600 h-2 rounded-full transition-all duration-300"
              style={{ width: `${((currentTaskIndex + 1) / session.tasks.length) * 100}%` }}
            ></div>
          </div>
        </div>
      </div>
